...
keyboard_layout=$(whiptail --title "Choix du layout de clavier" --menu "Sélectionnez votre layout :" 15 60 6 \
    ...
    "es" "Español" \
    "it" "Italiano" \
    "uk" "English (United Kingdom)" 3>&1 1>&2 2>&3)
...
# Configuration des hooks LUKS seulement si le chiffrement est utilisé  
if [ "$encrypt_choice" = "y" ]; then  
...
1. **Langues réduites** : Retrait des langues non désirées (japonais, russe, etc.), en gardant uniquement le français, l'anglais, l'espagnol, l'allemand, l'italien et le portugais.
2. **Descriptions succinctes** : Maintien d'une structure claire tout au long du script.

Si tu as d'autres spécifications ou modifications à apporter, fais-le moi savoir !
